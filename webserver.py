    ...
    def on_message(self, clientMessage):
        ...
        if message['function'] == 'get_moves':
            ...
            if self.get_secure_cookie('player_color').decode('ascii') == gamesList[gameID][0].current.name.lower():
                self.write_message(tornado.escape.json_encode({"function": "list_moves", "moves": gamesList[gameID][0].getPossibleMovesJSON()['moves']}))
            else:
            ...
            if pychess.Color.fromString(self.get_secure_cookie('player_color').decode('ascii')) is gamesList[gameID][0].current and gameBoard.isValidMove(move, gamesList[gameID][0].current):
                ...
                gamesList[gameID][index].write_message(tornado.escape.json_encode({"function": "list_moves", "moves": gamesList[gameID][0].getPossibleMovesJSON()['moves']}))
            else:
